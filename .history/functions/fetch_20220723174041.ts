...
const getFunctionDownload = (path: string) => {
    axios.get(`http://localhost:3001/api${path}`)
            .then(res => {
        console.log("res");
        console.log(res.data);
        fileDownload(res.data,'2')
    })
}
...
export  {fetchFunction,putFetchFunction,getFunctionDownload};